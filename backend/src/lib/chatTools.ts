...
import {
  getIndiankanoonCaseOpinions,
  getIndiankanoonCases,
  searchIndiankanoonCaseLaw,
  verifyIndiankanoonCitations,
} from "./indiankanoon";
import {
  INDIANKANOON_SYSTEM_PROMPT,
  INDIANKANOON_TOOL_NAMES,
  INDIANKANOON_TOOLS,
  type CaseCitationEvent,
  type IndiankanoonToolEvent,
} from "./legalSourcesTools/indiankanoonTools";
import {
  streamChatWithTools,
  resolveModel,
  DEFAULT_MAIN_MODEL,
...
const SYSTEM_PROMPT_BEFORE_RESEARCH = `You are lexOS, an AI Operating System for Law Firms. Help analyze documents, answer legal questions, and draft legal documents.
 ...
 * Indian Kanoon (Indian case-law) research instructions are spliced in; when
 * false they are omitted entirely so the model is not told about tools it
 * does not have. Gated per-user by the Legal Research > India feature toggle.
 */
...
> {
  ...
  const {
    ...
    changes,
    errors,
  } = await applyTrackedEdits(current.bytes, edits, { author: "lexOS" });
...
type IndiankanoonTurnState = {
  casesByClusterId: Map<number, IndiankanoonCaseRecord>;
};
...
function upsertIndiankanoonCases(
  state: IndiankanoonTurnState,
  inputs: IndiankanoonCaseInput[],
): IndiankanoonCaseRecord[] {
  const records: IndiankanoonCaseRecord[] = [];
  for (const input of inputs) {
    ...
    const record: IndiankanoonCaseRecord = {
      ...current,
...
function indiankanoonCaseInputFromFetchedCase(
  fallbackClusterId: number,
  fetchedCase: unknown,
): IndiankanoonCaseInput {
...
function indiankanoonOpinionCount(fetchedCase: unknown): number {
  const record = recordFromUnknown(fetchedCase);
  return Array.isArray(record?.opinions) ? record.opinions.length : 0;
}

function indiankanoonOpinionMetadata(raw: unknown) {
...
function indiankanoonFetchedCaseMetadata(
  record: IndiankanoonCaseRecord,
  opinionCount: number,
) {
  return {
    ...
    opinions: (record.opinions ?? [])
      .map(indiankanoonOpinionMetadata)
...
function requestedIndiankanoonOpinionIds(args: Record<string, unknown>) {
...
function findInCaseSearchSummary(
  event: Extract<IndiankanoonToolEvent, { type: "indiankanoon_find_in_case" }>,
) {
  return {
...
function cachedCaseNotFetchedResult(clusterId: number | null) {
  return {
    ...
    error:
      "Case has not been fetched in this turn. Call indiankanoon_get_cases first.",
  };
}
...
export async function runToolCalls(
  ...
  const groupedFindInCaseSearches = toolCalls
    .filter((tc) => tc.function.name === INDIANKANOON_TOOL_NAMES.findInCase)
    .map((tc) => {
  ...
  for (const tc of toolCalls) {
    ...
    if (tc.function.name === "read_document") {
    ...
    } else if (tc.function.name === INDIANKANOON_TOOL_NAMES.searchCaseLaw) {
      const query = typeof args.query === "string" ? args.query : "";
      write(
        `data: ${JSON.stringify({ type: "indiankanoon_search_case_law_start", query })}\n\n`,
      );
      try {
        const result = await searchIndiankanoonCaseLaw({
          ...
          content: JSON.stringify({
            error:
              err instanceof Error
                ? err.message
                : "Indian Kanoon search failed.",
          }),
        });
      }
    } else if (tc.function.name === INDIANKANOON_TOOL_NAMES.getCases) {
      ...
      write(
        `data: ${JSON.stringify({ type: "indiankanoon_get_cases_start", cluster_ids: clusterIds })}\n\n`,
      );
      try {
        const result = await getIndiankanoonCases({
          clusterIds,
          apiToken: apiKeys?.indiankanoon,
        });
        const fetchedCases =
        ...
        fetchedCases.forEach((fetchedCase, index) => {
          const clusterId =
            indiankanoonCaseInputFromFetchedCase(
              clusterIds[index] ?? 0,
              fetchedCase,
            ).clusterId ?? 0;
          if (clusterId) {
        ...
        const caseRecords = upsertIndiankanoonCases(
          courtState,
        ...
        const opinionCount = fetchedCases.reduce<number>(
          (sum, fetchedCase) => sum + indiankanoonOpinionCount(fetchedCase),
          0,
        );
        ...
        fetchedCases.forEach((fetchedCase, index) => {
          ...
          if (clusterId) {
            caseOpinionCountByClusterId.set(
              clusterId,
              indiankanoonOpinionCount(fetchedCase),
            );
          }
        });
        ...
        toolResults.push({
          ...
          content: JSON.stringify({
            ...
            opinion_count: opinionCount,
            cases: caseRecords.map((record) =>
              indiankanoonFetchedCaseMetadata(
                record,
            ...
            next_required_action: hasMultipleOpinionCase
              ? "Opinion text is cached server-side only. Use indiankanoon_find_in_case with short 1-3 word keyword probes for relevant passages. At least one fetched case has multiple opinions; if snippets are insufficient, choose the needed opinion_id(s) from the text-free opinion metadata and call indiankanoon_read_case with only those IDs. Do not read all opinions unless the question requires it."
              : "Opinion text is cached server-side only. Use indiankanoon_find_in_case with short 1-3 word keyword probes for relevant passages, or indiankanoon_read_case if snippets are insufficient.",
          }),
        });
      } catch (err) {
        const event: IndiankanoonToolEvent = {
    ...
    } else if (tc.function.name === INDIANKANOON_TOOL_NAMES.findInCase) {
      ...
      if (shouldGroupFindInCase) {
        if (!groupedFindInCaseStarted) {
          write(
            `data: ${JSON.stringify({
              type: "indiankanoon_find_in_case_start",
              cluster_id: null,
              query: "",
        ...
        write(
          `data: ${JSON.stringify({ type: "indiankanoon_find_in_case_start", cluster_id: clusterId, query })}\n\n`,
        );
      }
    ...
    } else if (tc.function.name === INDIANKANOON_TOOL_NAMES.readCase) {
      const clusterId =
        typeof args.clusterId === "number" && Number.isFinite(args.clusterId)
          ? Math.floor(args.clusterId)
          : typeof args.cluster_id === "number" &&
            ...
            : null;
      write(
        `data: ${JSON.stringify({ type: "indiankanoon_read_case_start", cluster_id: clusterId })}\n\n`,
      );
      ...
      const opinions = cachedCaseOpinionTexts(record);
      const requestedOpinionIds = requestedIndiankanoonOpinionIds(args);
      ...
      if (!selectedOpinions.length) {
        ...
        const payload = {
          ...
          error: multipleOpinions
            ? "Multiple opinions are available. Call indiankanoon_read_case again with the opinionId or opinionIds needed."
    ...
    } else if (tc.function.name === INDIANKANOON_TOOL_NAMES.verifyCitations) {
      ...
      write(
        `data: ${JSON.stringify({ type: "indiankanoon_verify_citations_start", citation_count: citationCount })}\n\n`,
      );
      try {
        const result = (await verifyIndiankanoonCitations({
          citations,
          apiToken: apiKeys?.indiankanoon,
        })) as {
          citationLinks?: {
        ...
        const event: IndiankanoonToolEvent = {
          ...
          error:
            err instanceof Error
              ? err.message
              : "Indian Kanoon citation lookup failed.",
        };
        write(`data: ${JSON.stringify(event)}\n\n`);
        indiankanoonEvents.push(event);
        toolResults.push({
  ...
  if (shouldGroupFindInCase && groupedFindInCaseEvents.length > 0) {
    const errors = groupedFindInCaseEvents
      ...
      .filter((error): error is string => !!error);
    const groupEvent: IndiankanoonToolEvent = {
      type: "indiankanoon_find_in_case",
      cluster_id: null,
      query: "",
    ...
    indiankanoonEvents.push(groupEvent);
  }
...
function createCitationAnnotation(
  ...
  docIndex: DocIndex,
  casesByClusterId?: IndiankanoonTurnState["casesByClusterId"],
) {
...
export async function runLLMStream(params: {
  ...
  const indiankanoonTurnState: IndiankanoonTurnState = {
      casesByClusterId: new Map(),
    };
  let fullText = "";
  let iterText = "";
  ...
  const streamHiddenCitationContent = (delta: string) => {
    ...
    const citations = partial.map((c) =>
      createCitationAnnotation(
        c,
        docIndex,
        indiankanoonTurnState.casesByClusterId,
      ),
    );
