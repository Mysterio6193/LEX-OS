...
const INDIANKANOON_CITATION_REMINDER_TOOL_NAMES = new Set([
  "indiankanoon_find_in_case",
  "indiankanoon_read_case",
]);
const INDIANKANOON_CITATION_REMINDER = `INDIAN CASE LAW CITATION REMINDER:
If your final answer relies on any Indian Kanoon case, every such case reference must have BOTH a clickable markdown case link and an inline [N] marker.
...
{"ref": N, "cluster_id": 123, "quotes": [{"opinion_id": 123, "quote": "exact verbatim opinion text"}]}.
Do not use doc_id, page, top-level quote, case_name, or citation fields for Indian Kanoon case entries.`;
...
function responseInstructions(systemPrompt: string, includeReminder: boolean) {
  return includeReminder
    ? `${systemPrompt}\n\n${INDIANKANOON_CITATION_REMINDER}`
    : systemPrompt;
}

function shouldAppendIndiankanoonCitationReminder(call: NormalizedToolCall) {
  return INDIANKANOON_CITATION_REMINDER_TOOL_NAMES.has(call.name);
}
...
export async function streamOpenAI(
  ...
  let fullText = "";
  let needsIndiankanoonCitationReminder = false;
    ...
    for (let iter = 0; iter < maxIter; iter++) {
      ...
      const response = await createResponse({
        ...
        instructions: responseInstructions(
          systemPrompt,
          needsIndiankanoonCitationReminder,
        ),
        input,
        tools: responseTools,
        stream: true,
      ...
      if (toolCalls.some(shouldAppendIndiankanoonCitationReminder)) {
        needsIndiankanoonCitationReminder = true;
      }
