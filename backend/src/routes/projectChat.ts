...
import { getDocumentPlainText } from "../lib/rag/ingest";
...
LIMITATION:
When the user asks about limitation, the last date to file, or whether a claim is time-barred for a recognised Indian cause, call compute_limitation with the trigger (cause-of-action) date and the limitation type. If unsure of the type key, call it with an empty key first to get the list. Report the computed due date and days remaining, state the statutory basis, flag any condonation caveat, and offer to record it with save_deadline. Always say the result is advisory and that the advocate must verify (exclusions, acknowledgements, court holidays, and facts can change it).

TIME & BILLING:
This matter tracks billable time. When the user mentions time spent on work (e.g. "spent 45 minutes reviewing the SPA", "log an hour for the client call"), call save_time_entry with a short description and the time converted to minutes. The hourly rate defaults to the firm's configured rate. Do not announce that you logged time; the UI shows it. Invoices (with GST) are generated by the user in the Billing tab.
...
projectChatRouter.post("/", requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (agentMode) {
            // Citation guard: verify document citations' quotes actually
            // appear in the cited source. (Case-law quote text isn't
            // available post-stream, so case cites are noted, not counted.)
            const docCites: CitationToCheck[] = [];
            for (const raw of annotations as Record<string, unknown>[]) {
                if (raw.kind === "case") continue;
                const quotes: string[] = [];
                if (Array.isArray(raw.quotes)) {
                    for (const q of raw.quotes) {
                        const qq = (q as { quote?: unknown })?.quote;
                        if (typeof qq === "string") quotes.push(qq);
                    }
                }
                if (typeof raw.quote === "string") quotes.push(raw.quote);
                docCites.push({
            ...
            if (docCites.length) {
                ...
                for (const id of ids) {
                    const t = await getDocumentPlainText(id, db);
                    if (t) sourceText.set(id, t);
                }
                unverifiedCount = verifyCitations(docCites, sourceText).filter(
                    (v) => !v.verified,
                ).length;
            }
            ...
            verification.notes = `${verification.notes} ${CITATION_COVERAGE_NOTE}`;
