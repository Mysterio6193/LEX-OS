...
type UserProfileRow = {
    ...
    legal_research_in: boolean | null;
};
...
const PROFILE_SELECT =
    "display_name, organisation, message_credits_used, credits_reset_date, tier, title_model, tabular_model, mfa_on_login, legal_research_in";
...
async function selectProfile(
    ...
    if (legacy.data && typeof legacy.data === "object") {
        ...
        if (!("legal_research_in" in row)) {
            Object.assign(row, { legal_research_in: true });
        }
    }
    return legacy;
}
...
function serializeProfile(row: UserProfileRow, apiKeyStatus?: ApiKeyStatus) {
    ...
    return {
        ...
        legalResearchIn: row.legal_research_in !== false,
...
function validateProfilePayload(body: unknown):
    ...
    if ("legalResearchIn" in raw) {
        if (typeof raw.legalResearchIn !== "boolean") {
            return {
                ok: false,
                detail: "legalResearchIn must be a boolean",
            };
        }
        update.legal_research_in = raw.legalResearchIn;
    }
