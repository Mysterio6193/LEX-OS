...
const indiankanoonOpinionsCache = new Map<number, CaseLawOpinion[]>();
const caseOpinionsRequestCache = new Map<
    string,
    ReturnType<typeof getIndiankanoonOpinions>
>();
...
const CASE_OPINION_SANITIZER_CONFIG = {
    ...
    ALLOWED_URI_REGEXP: /^(?:https:\/\/indiankanoon\.org\/|#)/i,
    FORBID_ATTR: ["style"],
    FORBID_TAGS: [
        "embed",
        "form",
...
function friendlyCaseError(message: string): string {
    ...
    if (message.includes("429") || /rate limit|throttled/i.test(message)) {
        ...
        return wait
            ? `Indian Kanoon is rate limiting requests. Please try again in about ${wait} seconds.`
            : "Indian Kanoon is rate limiting requests. Please try again shortly.";
    }
    if (message.includes("401") || /credentials|token|auth/i.test(message)) {
        return "Indian Kanoon authentication is not configured correctly.";
    }
    return "Could not load this case from Indian Kanoon. Please try again shortly.";
}
...
export function CaseLawPanel({
    ...
    const cachedOpinions = indiankanoonOpinionsCache.get(tab.clusterId);
    ...
    useEffect(() => {
        ...
        const cached = indiankanoonOpinionsCache.get(tab.clusterId);
        if (cached?.length) {
            setOpinions(cached);
            setLoading(false);
            setError(null);
        ...
        if (!request) {
            request = getIndiankanoonOpinions(tab.clusterId).finally(() => {
        ...
        request
            .then((nextOpinions) => {
                if (!cancelled) {
                    ...
                    indiankanoonOpinionsCache.set(tab.clusterId, nextOpinions);
                }
            })
            .catch((err: unknown) => {
                if (!cancelled) {
                ...
                <div className="flex min-w-0 shrink flex-wrap items-center justify-end gap-2">
                    ...
                    {courtlistenerUrl && (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            aria-label="Open in Indian Kanoon"
