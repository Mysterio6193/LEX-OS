...
function toolCallLabel(name: string): string {
    ...
    if (name === "indiankanoon_search_case_law")
        return "Searching case law...";
    if (name === "indiankanoon_get_cases") return "Fetching cases...";
    if (name === "indiankanoon_find_in_case") return "Searching case...";
    if (name === "indiankanoon_read_case") return "Reading case...";
    if (name === "indiankanoon_verify_citations")
...
function internalCaseHref(
    ...
    if (typeof value === "number") return `in-case-${value}`;
    if (!value) return null;
    const match = value.match(/^in-case-(\d+)$/);
    return match ? `in-case-${match[1]}` : null;
}
    ...
    return (
        ...
        >
            <ReactMarkdown
                ...
                urlTransform={(url) =>
                    /^in-case-\d+$/.test(url) ? url : defaultUrlTransform(url)
                }
                components={{
                    table: ({ node, ...props }) => (
...
export function AssistantMessage({
    ...
    const renderEvent = (
        ...
        if (event.type === "indiankanoon_search_case_law") {
        ...
        if (event.type === "indiankanoon_get_cases") {
            ...
            const items: IndianKanoonBlockItem[] =
                ...
                event.cluster_ids.map((clusterId) => {
                    const citation = caseCitations.get(`in-case-${clusterId}`);
                    return {
        ...
        if (event.type === "indiankanoon_find_in_case") {
            ...
            if (searches.length > 0) {
                ...
                const items: IndianKanoonBlockItem[] = searches.map(
                    (search) => ({
        ...
        if (event.type === "indiankanoon_read_case") {
        ...
        if (event.type === "indiankanoon_verify_citations") {
